                ...
                if (snapshot.hasData) {
                  ...
                  return Expanded(
                    child: ListView.builder(
                      ...
                      // shrinkWrap: true,
                      itemCount: menuList != null ? menuList.length : 1,
                        ...
                        if (menuList == null) {
                          ...
                          return Container(
                            ...
                            child: Padding(
                              ...
                              child: TextObject("메뉴가 존재하지 않습니다", Colors.black54,
                                  15, FontWeight.w300,
                                  center: false),
                            ),
                          );
...
Future<Map<String, dynamic>?> getStoreInfo(String test) async {
  try {
    ...
    var response =
        await upload.get("http://$ip/database/getStore", queryParameters: data
            // data: formData,
...
Future<Map<String, dynamic>?> getMenuInfo(String name) async {
  try {
    ...
    var response =
        await upload.get("http://$ip/database/getMenu", queryParameters: data
            // data: formData,
            // options: dio.Options(headers: {'Connection': 'keep-alive'}),
            );
    // print(2);
...
Future<Map<String, dynamic>?> getReview(String name, String engine) async {
  try {
    ...
    String url;
    if (engine == "네이버") {
      url = "http://$ip/database/review/naver";
    } else if (engine == "에브리타임") {
      url = "http://$ip/database/review/everytime";
    } else {
      url = "http://$ip/database/review/community";
      // return null;
    }
